function [R_est, t_est] = dlsu_pnpu_2stage(X, x, K, Sigmas, Sigmas2D, R_est_0, t_est_0)
%poses = dlsu(X, x, Xs, Xe, l, Sigmas, SigmasLines, Sigmas2D, SigmasLines2D, R_est, t_est)
    ...
    if (size(R_est, 3) == 1)
        [R_est_2, t_est_2] = dlsu(X, xn, zeros(3,0), zeros(3,0), zeros(3, 0), Sigmas_n, zeros(6,6,0), Sigmas2D, zeros(3,3,0), R_est, t_est);
        if (size(R_est_2, 3) == 1)
            R_est = R_est_2;
            t_est = t_est_2;
        end
