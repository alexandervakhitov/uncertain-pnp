function [R_est, t_est] = dlsu_plain_pnpu(X, x, K, Sigmas, Sigmas2D, R_est_3, t_est_3, mode)
    if nargin < 8
        mode = 1;
    end
...
%     Sigmas2D = Sigmas2D / K(1,1)/K(1,1);
    ...
    xn = x;
    [R_est, t_est] = dlsu_fast(X, xn, zeros(3,0), zeros(3,0), zeros(3, 0), sigmas2, zeros(6,6,0), sigmas2d_2, zeros(3,3,0), R_est_3, t_est_3, 0);
    if (size(R_est, 3) == 0)
        cnt = 0;
        while cnt < 5 && size(R_est, 3) == 0
            ...
            [R_est, t_est] = dlsu_fast(Rr*X, xn, zeros(3,0), zeros(3,0), zeros(3, 0), sigmas2, zeros(6,6,0), sigmas2d_2, zeros(3,3,0), R_est_3*Rr', t_est_3, 0);
            cnt = cnt+1;
        end
    ...
    if size(R_est,3)>1
        [R_est_2, t_est_2] = dlsu(X, xn, zeros(3,0), zeros(3,0), zeros(3, 0), Sigmas0, zeros(6,6,0), Sigmas2D_0, zeros(3,3,0), R_est(:,:,1), t_est(:,1), 0);
