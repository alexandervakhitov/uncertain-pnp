function [R,t,Xc,best_solution]=efficient_pnp_planar_gauss_lines1(x3d_h,x2d_h,Xs, Xe, xs, xe, A, l2d)
...
THRESHOLD_REPROJECTION_ERROR = 1;%5/800;%error in degrees of the basis formed by the control points. 
...
Xtot = [Xw Xs Xe];
[Alph_T, Alph_Tc, m_tot] = epnp_orig.compute_alphas_planar(Xtot, Cw);

[Utotc,Stotc, ~] = svd(Alph_Tc*Alph_Tc');
RA_tot = Utotc*Stotc(1:3,1:3)*Utotc'*Cw;
atotc_svals = sqrt(diag(Stotc(1:3, 1:3))');
...
[Cc,Xc,sc] = epnp_lines.compute_norm_sign_scaling_factor2All(X1, Alph, Alph_T, norm_cw_tot, SUac_tot);
[R,T] = epnp_lines.getrotTPlanar(XT_mean, RA_tot, Cc, m_tot);
...
[Cc,Xc,sc] = epnp_lines.compute_norm_sign_scaling_factor2All(X2, Alph, Alph_T, norm_cw_tot, SUac_tot);
[R,T] = epnp_lines.getrotTPlanar(XT_mean, RA_tot, Cc, m_tot);
...
%     end
    ...
    [Cc,Xc,sc] = epnp_lines.compute_norm_sign_scaling_factor2All(X3, Alph, Alph_T, norm_cw_tot, SUac_tot);
    [R,T] = epnp_lines.getrotTPlanar(XT_mean, RA_tot, Cc, m_tot);
...
% end
is_planar = 1;
[Xc_opt,R_opt,T_opt,err_opt,iter] = epnp_lines.optimize_betas_gauss_newton2(Kernel,...
    Cw,Beta0,Xs,Xe,xs,xe,A,Xw, U,XT_mean, RA_tot, m_tot, Alph_T, is_planar);
