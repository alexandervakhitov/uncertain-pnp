function [R,t,Xc,best_solution]=efficient_pnp_planar_gauss(x3d_h,x2d_h,A)
...
%Compute M
MtM = epnp_orig.compute_M1_planar(U,Alph,A);

%Compute kernel M
[V,S]=eig(MtM);

inds = [1 4 7 2 5 8 3 6 9];
Km=V(inds,3:-1:1); %in matlab we have directly the funcion km=null(M);

%1.-Solve assuming dim(ker(M))=1. X=[Km_end];------------------------------
dim_kerM=1;
X1=Km(:,end);
[Cc,Xc,sc]=compute_norm_sign_scaling_factor2(X1,Cw,Alph,Xw);

%[R,T]=getrotTPlanar(Xw,Xc);  %solve exterior orientation
[R, T] = epnp_orig.getrotTPlanar(Xw,Xc);
...
[Cc,Xc,sc] = compute_norm_sign_scaling_factor2(X2,Cw,Alph,Xw);

[R,T] = epnp_orig.getrotTPlanar(Xw,Xc);  %solve exterior orientation
...
if min(err)>THRESHOLD_REPROJECTION_ERROR %just compute if we do not have good solution in the previus cases
    ...
    [Cc,Xc,sc] = compute_norm_sign_scaling_factor2(X3,Cw,Alph,Xw);
...
Beta0=Betas/sc;
[Xc_opt,R_opt,T_opt,err_opt,iter] = epnp_orig.optimize_betas_planar_gauss_newton(Kernel,Cw,Beta0,Alph,Xw,U,A);
