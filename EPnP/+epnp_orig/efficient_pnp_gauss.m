...
THRESHOLD_REPROJECTION_ERROR=0.020;%error in degrees of the basis formed by the control points. 
...
%points centroid)
Cw = epnp_orig.define_control_points();

%compute alphas (linear combination of the control points to represent the 3d
%points)
Alph = epnp_orig.compute_alphas(Xw,Cw);

%Compute M
M = epnp_orig.compute_M_ver2(U,Alph,A);
...
dim_kerM=1;
X1=Km(:,end);
[Cc,Xc,sc] = epnp_orig.compute_norm_sign_scaling_factor(X1,Cw,Alph,Xw);

[R,T] = epnp_orig.getrotT(Xw,Xc);  %solve exterior orientation
err(1) = epnp_orig.reprojection_error_usingRT(Xw,U,R,T,A);
...
[Cc,Xc,sc] = epnp_orig.compute_norm_sign_scaling_factor(X2,Cw,Alph,Xw);

[R,T] = epnp_orig.getrotT(Xw,Xc);  %solve exterior orientation
err(2) = epnp_orig.reprojection_error_usingRT(Xw,U,R,T,A);
...
if min(err)>THRESHOLD_REPROJECTION_ERROR %just compute if we do not have good solution in the previus cases
    ...
    [Cc,Xc,sc]=epnp_orig.compute_norm_sign_scaling_factor(X3,Cw,Alph,Xw);
  
    [R,T]=getrotT(Xw,Xc);  %solve exterior orientation
    err(3)=epnp_orig.reprojection_error_usingRT(Xw,U,R,T,A);
...
if min(err)>THRESHOLD_REPROJECTION_ERROR %just compute if we do not have good solution in the previus cases
    ...
    [Cc,Xc,sc]=epnp_orig.compute_norm_sign_scaling_factor(X4,Cw,Alph,Xw);
...
Beta0=Betas/sc;
[Xc_opt,R_opt,T_opt,err_opt,iter]=epnp_orig.optimize_betas_gauss_newton(Kernel,Cw,Beta0,Alph,Xw,U,A);
