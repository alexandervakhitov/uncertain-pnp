function [R_est, t_est] = epnpu_pnpu_2stage(X, x, K, Sigmas3D, Sigmas2D, R_est_0, t_est_0)
...
%function [R,T,Xc,best_solution,opt]=efficient_pnpu_gauss(x3d_h,x2d_h,A,Sigmas3D,Sigmas2D,Rest,test,is_pca)    
    ...
    if (size(R_est, 3) == 1)
        [R_est_2,t_est_2,Xc,best_solution,opt]=efficient_pnpu_gauss(x3d_h,x2d_h,K,Sigmas3D_n, Sigmas2D, R_est, t_est, true);
        if (size(R_est_2,3) == 1)
            R_est = R_est_2;
            t_est = t_est_2;
        end
