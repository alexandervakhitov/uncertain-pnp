function [R_est, t_est] = epnpu2_pnpu(X, x, K, Sigmas3D, Sigmas2D, R_est_3, t_est_3, mode)
    if nargin < 8
        mode = 2;
        m2 = 1;
    end
...
%function [R,T,Xc,best_solution,opt]=efficient_pnpu_gauss(x3d_h,x2d_h,A,Sigmas3D,Sigmas2D,Rest,test,is_pca)        
    ...
    [R_est,t_est,Xc,best_solution,opt]=efficient_pnpu_gauss(x3d_h,x2d_h,K,Sigmas3D, Sigmas2D, R_est, t_est, true);    
