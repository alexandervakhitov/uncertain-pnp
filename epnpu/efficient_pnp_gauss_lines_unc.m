function [R,T,Xc,best_solution,opt,t1,t2,t3,t4]=efficient_pnp_gauss_lines_unc(x3d_h, x2d_h, ...
    Xs, Xe, xs, xe, A, l2d, Sigmas3D, Sigmas2D, Sigmas3DLines, Sigmas2DLines, Rest, test, is_pca)
...
if is_pca && size(Xw,2)<-1
    %Cw=define_control_points_pca(Xw');
    Cw=epnp_unc.define_control_points_pca_unc(Xw', Sigmas3D);
else
...
t0 = tic;
%Compute M
M = epnp_unc.compute_M_uncertain_lines(U,Alph,A, Sigmas3D, Sigmas2D, Cw, Rest, test, Alph_S, Alph_E, l2d, Sigmas3DLines, Sigmas2DLines);
