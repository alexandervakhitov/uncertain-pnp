function [R,T,Xc,best_solution,opt,t1,t2,t3,t4]=efficient_pnpu_gauss_lines(x3d_h, x2d_h, A, ...
    Sigmas3D, Sigmas2D, Xs, Xe, l2d, Sigmas3DLines, Sigmas2DLines, Rest, test, is_pca)
...
[Alph, Alph_c, mc] = epnp_lines.compute_alphas(Xw, Cw);
[Alph_S, Alph_Sc, ms] = epnp_lines.compute_alphas(Xs, Cw);
[Alph_E, Alph_Ec, me] = epnp_lines.compute_alphas(Xe, Cw);
...
t0 = tic;
%Compute M
% M = epnp_unc.compute_M_uncertain(U',Alph',A, Sigmas3D, Sigmas2D, Cw, Rest, ...
...
err(1) = epnp_unc.reprojection_error_usingRTAll(Xw, U, Xs,Xe,l2d,R,T,A);
...
[R,T] = epnp_lines.getrotT(XT_mean, RA_tot, Cc, m_tot);
err(2) = epnp_unc.reprojection_error_usingRTAll(Xw, U, Xs,Xe,l2d,R,T,A);
...
%     end
    ...
    [R,T] = epnp_lines.getrotT(XT_mean, RA_tot, Cc, m_tot);
    err(3) = epnp_unc.reprojection_error_usingRTAll(Xw, U, Xs,Xe,l2d,R,T,A);
...
%     end
    ...
    [R,T] = epnp_lines.getrotT(XT_mean, RA_tot, Cc, m_tot);
    err(4) = epnp_unc.reprojection_error_usingRTAll(Xw, U, Xs,Xe,l2d,R,T,A);
...
[Xc_opt,R_opt,T_opt,err_opt,iter] = epnp_unc.optimize_betas_gauss_newton2(Kernel,...
    Cw,Beta0,Xs,Xe,l2d,A,Xw,U, XT_mean, RA_tot, m_tot,Alph_T);
